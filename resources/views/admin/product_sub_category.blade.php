...
<script>
    ...
    var table = $('#dataTable1').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": "{{route('admin.all-sub-category.list')}}",
                "type":"get",
                data: function (d)
                {
            ...
            },
            {
               "mRender":function(data,type,row)
               {
                    if(row.child_categories.length === 0 && row.products.length === 0)
                    return `<a href="/admin/product-sub-category/${row.id}/edit" class="btn btn-primary btn-sm"><i class="fa fa-edit" aria-hidden="true"></i></a>  <a href="javascript:;" data-toggle="modal" data-target="#deleteModal" class="btn btn-danger btn-sm" onclick="deleteData(${row.id})"><i class="fa fa-trash" aria-hidden="true"></i></a>`
                    else
                    return `<a href="/admin/product-sub-category/${row.id}/edit" class="btn btn-primary btn-sm"><i class="fa fa-edit" aria-hidden="true"></i></a>  <a href="javascript:;" data-toggle="modal" data-target="#canNotDeleteModal" class="btn btn-danger btn-sm" disabled><i class="fa fa-trash" aria-hidden="true"></i></a>`
               }
            }
        ]
        });
